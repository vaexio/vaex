...
def test_cat_string():
	...
	ds = ds0.categorize('colors')#, ['red', 'green'], inplace=True)
	assert ds.iscategory('colors')
	assert ds.limits('colors', shape=128) == ([-0.5, 2.5], 3)

	ds = ds0.categorize('colors', values=['red', 'green'])
	assert ds.iscategory('colors')
...
def test_count_cat():
	...
	ds = ds0.categorize(ds0.colors)
	ds = ds0.categorize(ds0.names)	

	ds = ds0.categorize('colors', ['red', 'green', 'blue'])
	assert ds.count(binby=ds.colors).tolist() == [1, 2, 1]
	ds = ds0.categorize('colors', ['red', 'blue', 'green', ], inplace=True)
