...
int main(int argc, char *argv[]){
  ...
  double musec;
  int dimstart=atoi(argv[4]),dimend=numcols,dims,*dumy,numspaces,comb[30][50000];
  int k=0,l=1,m=2;
//-------------------------
...
//     sy1=((*(sdata[d[1]]+(int)ceil(numdata*0.80))-*(sdata[d[1]]+(int)ceil(numdata*0.20))))/log(numdata);
    ...
    ImagePGMBinWrite2D(&density,fn);
    quality[i]=MaxTreeforSpaces(fn,4,&numLeaves[i]);
    printf("%lf\n",quality[i]);
exit_map2d (&density);
}
...
printf("[%s]:%lf\n",header[index[loop]],quality[loop]);
 free(density1.map);
    ...
    FILE *outfile = fopen("rankingScott.txt","a");
...
//{  //printf("correlated\n");
  ...
  sx1=10.0*((*(sdata[d[0]]+(int)ceil(numdata*0.80))-*(sdata[d[0]]+(int)ceil(numdata*0.20))))/log(numdata);
    sy1=10.0*((*(sdata[d[1]]+(int)ceil(numdata*0.80))-*(sdata[d[1]]+(int)ceil(numdata*0.20))))/log(numdata);
    sz1=10.0*((*(sdata[d[2]]+(int)ceil(numdata*0.80))-*(sdata[d[2]]+(int)ceil(numdata*0.20))))/log(numdata);
...
if(sz1<0)
sz1=-sz1;
//     sx1=25.0*(maxima[d[0]]-minima[d[0]])/sqrt(numdata);
//     sy1=25.0*(maxima[d[1]]-minima[d[1]])/sqrt(numdata);
//     sz1=25.0*(maxima[d[2]]-minima[d[2]])/sqrt(numdata);
...
printf("3D density done\n");
  ...
  //  fclose(outfile);
	 ...
	 AVSdens3dwrite(4095,&density3,"density_deredR_logMass_lDensity.fld");
...
//        d[2] =tmp/10;
	...
	*(dataPCAcopy[j]+i)=*(dataPCA[i+1]+(j+1))*100;
      ...
      Compute3Ddensity(dataPCAcopy,numdata,prob,&numLeaves[nS]);
