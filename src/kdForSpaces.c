...
extern double MaxTreeforSpaces(char *imname,int CONNECTIVITY);
extern int combi(int N,int M,int *c);
...
printHeaders(int dimstart,double *quality,int *index,int numspaces,char **header)
{
   int i,j;
    pixel_qsort(quality,index,numspaces);
    printf("Quality of %dD Subspaces:\n",dimstart);
	...
	int tmp =index[i];
     ...
     {
      printf("[%s\tvs",header[tmp%10]);
      tmp =tmp/10;
     }
     printf("\t%s]\t:%lf\n",header[tmp],quality[i]);
    }
}
/*void Compute3Ddensity(double **data,map3d *density3,double max,double min,int numdata,int *prob)
{
double sx1,sy1,sz1,gmean;

init_map3d(&density3,minima[0],maxima[0],256,
	       minima[1],maxima[1],256,
	       minima[2],maxima[2],256);

    sx1=(maxima[k]-minima[k])/cbrt(numdata);
    sy1=(maxima[l]-minima[l])/cbrt(numdata);
    sz1=(maxima[m]-minima[m])/cbrt(numdata);

    //printf("%f\t%f\t%f\n", sx1, sy1,sz1);

    gmean = comp_data_probs_3d(&density3,sx1,sy1,sz1,numdata, data[0], data[1], data[2], prob);
    ...
    comp_density_3d ( &density3,sx1,sy1,sz1,gmean,numdata,data[0],data[1],data[2],prob);
...
int main(int argc, char *argv[]){
  ...
  double musec;
  int dimstart = 1,dims,*comb,numspaces;
  int k=0,l=1,m=2;
//-------------------------
...
parse(line,delim,header);
  ...
  for (j=0;j<numcols;j++){
    ...
    pixel_qsort (sdata[j],prob,numdata);
    printf("%s\n",header[j]);
  }
...
{    
	...
	sx1=(maxima[d[0]]-minima[d[0]])/sqrt(numdata);
	sy1=(maxima[d[1]]-minima[d[1]])/sqrt(numdata);
    ...
    ImagePGMBinWrite2D(&density,"density.pgm");
    quality[i]=MaxTreeforSpaces("density.pgm",4);
   // printf("%lf\n",quality[i]);

}
    printHeaders(dimstart,quality,comb,numspaces,header);
    dimstart=3;
    free(quality);
  } 
else if (dimstart==1){
 ...
 for (i=0;i<numspaces;i++)
{
    ...
    init_map3d(&density3,minima[d[0]],maxima[d[0]],256,
	       minima[d[1]],maxima[d[1]],256,
	       minima[d[2]],maxima[d[2]],256);
   ...
   // sz1=((*(sdata[m]+(int)ceil(numdata*0.80))-*(sdata[m]+(int)ceil(numdata*0.20))))/log(numdata);
    ...
    sz1=(maxima[d[2]]-minima[d[2]])/cbrt(numdata);
 ...
 //   AVSdens3dwrite(4095,&density3,"density.fld");
   ...
   //printf("%lf\n",quality[i]);
    ...
    //for(i=numspaces-1;i>0;i--)
    //printf("[%s\tvs\t%s]\t:%lf\n",header[index[i]/10],header[index[i]%10],quality[i]);
