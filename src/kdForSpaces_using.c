...
printHeaders(int dimstart,double *quality,int *index,int numspaces,char **header)
{
   int i,j;
	...
	int tmp =index[i];
     ...
     {
      printf("%s-",header[tmp%10]);
      tmp =tmp/10;
     }
     printf("%s]\t:%lf\n",header[tmp],quality[i]);
    }
}
// double Compute3Ddensity(double *pca1,double *pca2,double *pca3,map3d *density3,int numdata,int *prob)
// {
// double sx1,sy1,sz1,gmean,quality;
// 
...
int main(int argc, char *argv[]){
  ...
  double musec;
  int dimstart = 1,dims,*comb,*dumy,numspaces;
  int k=0,l=1,m=2;
//-------------------------
...
//        //do pca
//        
//        quality[i]=Compute3Ddensity(pca[0],pca[1],pca[2],densityGt3,numdata,prob)
// }      
//   
// }
}
